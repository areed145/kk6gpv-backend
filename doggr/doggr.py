...
class DownloadWorker(Thread):
    ...
    def run(self):
        while True:
            ...
            url = 'https://secure.conservation.ca.gov/WellSearch/Details?api=' + \
            ...
            county = re.findall(
                'County</label> <br />\s*(.*)<span>\s\[(.*)\]\s*</span>', page)[0][0]
            countycode = re.findall(
                'County</label> <br />\s*(.*)<span>\s\[(.*)\]\s*</span>', page)[0][1]
            district = int(re.findall(
                'District</label> <br />\s*(.*?)\s*</div', page)[0])
            operator = re.findall(
                'Operator</label> <br />\s*(.*)<span>\s\[(.*)\]\s*</span>', page)[0][0]
            operatorcode = re.findall(
                'Operator</label> <br />\s*(.*)<span>\s\[(.*)\]\s*</span>', page)[0][1]
            field = re.findall(
                'Field</label> <br />\s*(.*)<span>\s\[(.*)\]\s*</span>', page)[0][0]
            fieldcode = re.findall(
                'Field</label> <br />\s*(.*)<span>\s\[(.*)\]\s*</span>', page)[0][1]
            area = re.findall(
                'Area</label> <br />\s*(.*)<span>\s\[(.*)\]\s*</span>', page)[0][0]
            areacode = re.findall(
                'Area</label> <br />\s*(.*)<span>\s\[(.*)\]\s*</span>', page)[0][1]
            section = re.findall(
                'Section</label><br />\s*(.*?)\s*</div', page)[0]
            township = re.findall(
                'Township</label><br />\s*(.*?)\s*</div', page)[0]
            rnge = re.findall('Range</label><br />\s*(.*?)\s*</div', page)[0]
            bm = re.findall(
                'Base Meridian</label><br />\s*(.*?)\s*</div', page)[0]
            wellstatus = re.findall(
                'Well Status</label><br />\s*(.*?)\s*</div', page)[0]
            pwt = re.findall(
                'Pool WellTypes</label> <br />\s*(.*?)\s*</div', page)[0]
            spuddate = re.findall(
                'SPUD Date</label> <br />\s*(.*?)\s*</div', page)[0]
            gissrc = re.findall(
                'GIS Source</label> <br />\s*(.*?)\s*</div', page)[0]
            elev = re.findall('Datum</label> <br />\s*(.*?)\s*</div', page)[0]
            latitude = re.findall(
                'Latitude</label> <br />\s*(.*?)\s*</div', page)[0]
            longitude = re.findall(
                'Longitude</label> <br />\s*(.*?)\s*</div', page)[0]
            hh = {}
            hh['lease'] = lease
            hh['well'] = well
            hh['county'] = county
            ...
            for idx, i in enumerate(prod):
                ...
                if len(prod) > 0:
                    p['date'] = datetime.datetime.fromtimestamp(
                        int(re.findall('Date\(+(.*?)\)', i)[0][:-3])).strftime('%Y-%m-%d')
                    ...
                    p['method'] = re.findall(
                        'MethodOfOperation":+(.*?),', i)[0].replace('"', '')
                    p['waterdisp'] = re.findall(
                        'WaterDisposition":+(.*?),', i)[0].replace('"', '')
                    p['pwtstatus_p'] = re.findall(
                        'PWTStatus":+(.*?),', i)[0].replace('"', '')
                    p['welltype_p'] = re.findall(
                        'WellType":+(.*?),', i)[0].replace('"', '')
                    p['status_p'] = re.findall(
                        'Status":+(.*?),', i)[0].replace('"', '')
                    p['poolcode_p'] = re.findall(
                        'PoolCode":+(.*?),', i)[0].replace('"', '')
            ...
            for idx, i in enumerate(inj):
                ...
                if len(inj) > 0:
                    j['date'] = datetime.datetime.fromtimestamp(
                        int(re.findall('Date\(+(.*?)\)', i)[0][:-3])).strftime('%Y-%m-%d')
                    j['wtrstm'] = re.findall(
                        'WaterOrSteamInjected":+(.*?),', i)[0]
                    j['gasair'] = re.findall('GasOrAirInjected":+(.*?),', i)[0]
                    j['daysinj'] = re.findall(
                        'NumberOfDaysInjected":+(.*?),', i)[0]
                    j['pinjsurf'] = re.findall(
                        'SurfaceInjectionPressure":+(.*?),', i)[0]
                    j['wtrsrc'] = re.findall(
                        'SourceOfWater":+(.*?),', i)[0].replace('"', '')
                    j['wtrknd'] = re.findall(
                        'KindOfWater":+(.*?),', i)[0].replace('"', '')
                    j['pwtstatus_i'] = re.findall(
                        'PWTStatus":+(.*?),', i)[0].replace('"', '')
                    j['welltype_i'] = re.findall(
                        'WellType":+(.*?),', i)[0].replace('"', '')
                    j['status_i'] = re.findall(
                        'Status":+(.*?),', i)[0].replace('"', '')
                    j['poolcode_i'] = re.findall(
                        'PoolCode":+(.*?),', i)[0].replace('"', '')
                    if re.findall('YearlySum":+(.*?),', i)[0] != 'true':
